...
class ArrayInput
  ...
  def array_input_html(value, remove = true)
    button = if remove
               template.button_tag("-", "data-action": "remove", type: "button")
             else
               template.button_tag("+", "data-action": "add", type: "button")
             end
    template.content_tag(:div, class: "input-group--array__item #{options[:item_class]}") do
      template.text_area_tag("#{object_name}[#{method}][]",
                             value, class: "textarea-group--array #{options[:class]}") << button
    end
  end
end
